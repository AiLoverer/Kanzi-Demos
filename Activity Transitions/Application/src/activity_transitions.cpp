...
#if defined(ACTIVITY_TRANSITIONS_CODE_BEHIND_API) && !defined(ANDROID) && !defined(KANZI_CORE_API_IMPORT)
#include <Activity_transitions_code_behind_module.hpp>
#endif

using namespace kanzi;

class ActivityTransitions : public ExampleApplication
{
public:

    void onConfigure(ApplicationProperties& configuration) override
    {
        configuration.binaryName = "activity_transitions.kzb.cfg";
    }
...
#if defined(ACTIVITY_TRANSITIONS_CODE_BEHIND_API) && !defined(ANDROID) && !defined(KANZI_CORE_API_IMPORT)
        ActivityTransitionsCodeBehindModule::registerModule(getDomain());
#endif
...
{
    return new ActivityTransitions;
}
